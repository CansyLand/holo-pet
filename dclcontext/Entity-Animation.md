...
easingFunction: What easing function to use. See Non-linear tweens
...
loop (optional): If not provided, the sequence is only played once. If the field is present, the value must be a value of the TweenLoop enum. Accepted values are:
...
easingFunction: What easing function to use. See Non-linear tweens. In this format the value is required.
...
Instead of using the Tween component and letting the engine handle the transformation, you may prefer to do this transition incrementally, frame by frame, via a system in your scene. By moving the entity a small amount each time the function runs.
...
Move via system
The easiest way to move an entity is to gradually modify the position value stored in the Transform component.
...
💡 Tip: You can pass rotation values in euler degrees (from 0 to 360) by using Quaternion.fromEulerDegrees().
...
The example above defines a 3D path that's made up of four 3D vectors. The PathTransportData custom component holds the same data used by the custom component in the lerp example above, but adds a path array, with all of the points in our path, and a pathTargetIndex field to keep track of what segment of the path is currently in use.
...
easingFunction: What easing function to use. See Non-linear tweens. Note: This parameter is only used if a duration is provided.
...
Read more about texture tweens in the Texture Tweens section.
